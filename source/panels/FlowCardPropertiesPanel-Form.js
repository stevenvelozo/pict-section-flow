...
class FlowCardPropertiesPanelForm extends libPictFlowCardPropertiesPanel
{
	...
	render(pContainer, pNodeData)
	{
		...
		try
		{
			...
			if (this._Metacontroller && typeof this._Metacontroller.injectManifest === 'function')
			{
				// Deep clone the manifest so we can modify data addresses
				let tmpManifest = JSON.parse(JSON.stringify(this._Configuration.Manifest));

				// Point the form data at the node's Data object
				// The consuming application should set up AppData so that
				// node data is accessible at a known address
				this._Metacontroller.injectManifest(tmpManifest);
				this._InjectedSectionHash = tmpManifest.Hash || null;

				if (typeof this._Metacontroller.render === 'function')
				{
					this._Metacontroller.render();
				}
			}
			else
			{
